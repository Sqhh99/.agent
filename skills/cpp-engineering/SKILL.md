...
1. **读现有工程**：README、根/`CMakeLists.txt`、`CMakePresets.json`、`vcpkg.json`、`.clang-format`、已有目录和代码风格。完成：能说出项目类型与构建入口。
2. **判定画像**：Library / CLI / Qt GUI / Device / Algorithm / SDK；OS、编译器、C++ 标准、Qt 版本、CMake 版本、依赖管理器、测试框架。完成：画像写进任务笔记或回复。
3. **定范围**：现有规范优先。用户没要求重构就禁止扩大 diff。不改 `generated/`、`vendor/`、`third_party/` 源码。优先复用已有模块和依赖。
4. **设计接口再实现**：公开 API 先定契约（所有权、错误、线程、单位），再写代码。
5. **落地后必须真跑**：configure → build → 相关测试 → 能跑的静态检查 → 看 diff。完成：每步有真实命令输出。
6. **收尾**：按 `cpp-git-workflow` 分支/提交；需要发布再走 `cpp-ci-release`。
